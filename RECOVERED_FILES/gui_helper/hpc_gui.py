...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Tufts HPC Jupyter Launcher")
    ...
    def _build(self):
        ...
        row += 1
        ttk.Button(frm, text="Start Jupyter on HPC (auto)", command=self.start_jupyter).grid(row=row, column=0, sticky=tk.W)
        ...
        ttk.Button(frm, text="Check Disk & Resources", command=self.check_resources).grid(row=row, column=2, sticky=tk.W)
        ttk.Button(frm, text="Copy Resources Cmds", command=self.copy_resources_cmds).grid(row=row, column=3, sticky=tk.W)
        ...
        row += 1
        ttk.Button(frm, text="Copy Jupyter Setup Cmds", command=self.copy_setup_cmds).grid(row=row, column=0, sticky=tk.W)
        ttk.Button(frm, text="Copy Auto Start Script", command=self.copy_auto_start_script).grid(row=row, column=1, sticky=tk.W)
        ttk.Button(frm, text="Detect Node", command=self.detect_node).grid(row=row, column=2, sticky=tk.W)
        ttk.Button(frm, text="Copy Browser URL", command=self.copy_browser_url).grid(row=row, column=3, sticky=tk.W)
        ...
        row += 1
        tips = (
            "Workflow: Start Jupyter → copy NODE → Open Tunnel → use URL in Cursor\n"
            "Pitfall: /cluster commands must run on HPC node, not locally.\n"
            "Cursor URL: http://localhost:<port>/?token=<token>"
        )
